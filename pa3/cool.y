...
%{
  ...
  for the location of tokens */
    ...
    */
    ...
    %type <formal>   formal
    %type <expressions> expr_plus_list
    ...
    */
    ...
    parse_results = $$; }
    | class class_list 
    { $$ = append_Classes(single_Classes($1), $2); parse_results = $$; }
    parse_results = $$; }
    ;
    ...
    | expr_plus_list ',' expr { $$ = append_Expressions($1, single_Expressions($3)); }

    expr_statements:
    expr ';' { $$ = single_Expressions($1); }
    | expr ';' expr_statements { $$ = append_Expressions(single_Expressions($1), $3); }
    | error ';' expr_statements

    let_:
    OBJECTID ':' TYPEID IN expr { $$ = let($1, $3, no_expr(), $5); }
    | OBJECTID ':' TYPEID ASSIGN expr IN expr { $$ = let($1, $3, $5, $7); } 
    | OBJECTID ':' TYPEID ',' let_ { $$ = let($1, $3, no_expr(), $5); }
    | OBJECTID ':' TYPEID ASSIGN expr ',' let_ { $$ = let($1, $3, $5, $7); }
    | error ',' let_

    expr: 
    OBJECTID ASSIGN expr { $$ = assign($1, $3); } 
    | OBJECTID '('  ')'  { $$ = dispatch( object(idtable.add_string("self")) , $1, nil_Expressions()); }
    | OBJECTID '('  expr_plus_list  ')' { $$ = dispatch(object(idtable.add_string("self")), $1, $3); }
    | expr '.' OBJECTID '(' ')' { $$ = dispatch($1, $3,nil_Expressions()); }
    | expr '@' TYPEID '.' OBJECTID '(' ')' { $$ = static_dispatch($1, $3, $5, nil_Expressions()); }
    | expr '.' OBJECTID '(' expr_plus_list ')' { $$ = dispatch($1, $3, $5); }
    | expr '@' TYPEID '.' OBJECTID '(' expr_plus_list ')' { $$ = static_dispatch($1, $3, $5, $7); }
    | LET let_ {$$ = $2;}
    | IF expr THEN expr ELSE expr FI { $$ = cond($2, $4, $6);  }
    | CASE expr OF branch_list ESAC { $$ = typcase($2, $4); }
    | WHILE expr LOOP expr POOL { $$ = loop($2, $4); }
    | '{' expr_statements   '}'  { $$ = block($2);   } 
    | '{' error '}'
    | NEW TYPEID    { $$ = new_($2);     }
